...
const App = () => {
  ...
  const myPigLatinCodeHere = () => {
    ...
    const translatedWordsArray = arrayOfUserInput.map(eachWord => {
      ...
      if(firstVowelLocation === 0) {
        ...
        const whatGetsMoved = eachWord.slice(0, y)
        const restOfWord = eachWord.slice(y)
        return `${restOfWord}${whatGetsMoved}ay`
      } else {
        const whatGetsMoved = eachWord.slice(0, firstVowelLocation)
        const restOfWord = eachWord.slice(firstVowelLocation)
        return `${restOfWord}${whatGetsMoved}ay`
      }
    })
  ...
  return (
    <>
      ...
      <p>{inputTranslated}</p>
      <footer>&copy; 2022 | Coded by: Sarah</footer>
    </>
  )
}
